...
const proposeSchema = z.object({
  bookId: z.string(),
  action: z.enum(["brainstorm_tab", "continue_chapter", "expand_card", "generate_summary", "contradiction_check", "extract_entities", "critique_prose", "generate_story_profile", "check_story_structure"]),
  scope: z.object({
...
const TASK_PROMPTS: Record<string, string> = {
  ...
  check_story_structure: "You are checking whether THIS CHAPTER (not the whole book) actually functions as a piece of story, using the Study of Story guide's Foundations vocabulary — a structural check, not a prose/voice critique (that's a separate tool). Evaluate exactly these five things, based only on what's actually in the chapter text, not what you'd expect a chapter to have: (1) Event — does something actually happen; (2) Causality — does something happen BECAUSE something else happened, not just in sequence; (3) Change — is there a real before/after, and is it External (the situation/world is different), Internal (a character's knowledge/belief/resolve changed, even if the world didn't), Both, or None; (4) Resolution mode — does the chapter end Resolved (new equilibrium), Unresolved (deliberate disequilibrium), Near-static (mood/observation, minimal net change), or Incomplete/ongoing (it's a fragment, not a real ending point); (5) Conflict/resistance — what makes the change non-trivial (doesn't have to be a villain — nature, time, ignorance, a system, a contradiction all count). For each, say whether it's present and give a one-line note citing something specific from the chapter, not a generic template answer. Absence of one of these is not automatically bad — some scenes are deliberately near-static — but say so plainly rather than fabricating an event that isn't there. If a STORY PROFILE is present above, also add up to 3 short alignmentNotes flagging anywhere this chapter seems to drift from it (e.g. Structure, Cast roles, whose Change this was 'supposed' to be) — only real observations, not one manufactured per category. Return ONLY this JSON object, no commentary: {\"foundations\":{\"event\":{\"present\":true|false,\"note\":\"...\"},\"causality\":{\"present\":true|false,\"note\":\"...\"},\"change\":{\"mode\":\"External\"|\"Internal\"|\"Both\"|\"None\",\"note\":\"...\"},\"resolution\":{\"mode\":\"Resolved\"|\"Unresolved\"|\"Near-static\"|\"Incomplete/ongoing\",\"note\":\"...\"},\"conflict\":{\"present\":true|false,\"note\":\"...\"}},\"alignmentNotes\":[\"...\"]}",
};
...
export async function POST(req: NextRequest) {
  ...
  const structuredActions = new Set(["brainstorm_tab", "contradiction_check", "expand_card", "extract_entities", "critique_prose", "generate_story_profile", "check_story_structure"]);
  ...
  const OBJECT_ACTIONS = new Set(["expand_card", "extract_entities", "generate_story_profile", "check_story_structure"]); // model returns {...}
