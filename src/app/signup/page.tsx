...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (error) {
      // Supabase returns "Signups are not allowed for this project" when disabled
      if (error.message.toLowerCase().includes("not allowed") || error.message.toLowerCase().includes("disabled")) {
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#0c0c0e] px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-lg border border-[#232328] bg-[#141417] p-6">
          <h1 className="mb-1 text-lg font-semibold text-[#f4f4f5]">Create account</h1>
          <p className="mb-5 text-sm text-[#9b9ba4]">Start your writing workspace.</p>
          ...
          <p className="mt-5 text-center text-sm text-[#9b9ba4]">
            ...
            <a href="/login" className="text-[#818cf8] hover:underline">
              Sign in
            </a>
          </p>
        </div>
